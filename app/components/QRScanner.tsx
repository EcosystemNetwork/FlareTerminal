...
const QRScanner: React.FC<QRScannerProps> = ({ onScan, onError }) => {
  ...
  useEffect(() => {
    const getDevices = async () => {
      try {
        ...
        if (onError) onError('Failed to enumerate devices: ' + (err as Error).message);
      }
    };
  ...
  const startScanner = () => {
    ...
    scannerRef.current.start(
      ...
      if (onError) onError('Failed to start scanner: ' + err);
    });
  };
  ...
  const stopScanner = () => {
    if (scannerRef.current && scannerRef.current.isScanning) {
      scannerRef.current.stop().catch(err => {
        ...
        if (onError) onError('Failed to stop scanner: ' + err);
      });
    }
  };
